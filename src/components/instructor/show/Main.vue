<template>
    <div class="d-flex">
        <div class="info">
            <AvatarInput :disabled="true" :width="72" :height="72" :src="getSrc(instructor.avatar?.path)">
                <template #title>
                    <div class="ml-4">
            ...
            <div>
                ...
                {{ instructor.car?.brand.name }} {{ instructor.car?.model?.name }}
            </div>
        </div>
    </div>
</template>
    ...
    computed: {
        ...
        infos() {
            return [
                {
                    icon: "user.svg",
                    label: `${this.instructor?.name} ${this.instructor?.lastName}`
                },
                {
                    icon: "phone.svg",
                    label: `${this.instructor?.phoneNumber}`
                },
                {
                    icon: "message.svg",
                    label: `${this.instructor?.email}`
                },
                {
                    icon: "category.svg",
                    label: `${this.instructor?.category?.name}`
                },
            ]
        }
    },
