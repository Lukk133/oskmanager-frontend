...
export default {
    ...
    methods: {
        goTo(navElement) {
            if (navElement.name) {
                this.$router.push({ name: "StudentShow", params: { id: this.$route.params.id }, query: { tab: navElement.name } })
            } else {
                this.$router.push({ name: "StudentShow", params: { id: this.$route.params.id } })
